...
def create_grid(data, gridsize = 100):
    ...
    for feat_idx in range(data.shape[1]):
        ...
        int = ((feat_max - feat_min) / gridsize)
        linspace_list.append(np.arange(feat_min, feat_max, int, dtype = data.dtype))
...
def advsamples_pytorch(model, tr_data_im, tr_loader, args):
    ...
    from Tests.Models.blackbox_model_training.image.adv_boundary import generate_adverserial_for_class_wrapper
...
def sampledb_DBPS_binary(grid, model, decision_threshold = 0.5, n_samples_per_class = 100, batch_size = 1024, max_iter = 1e3, early_stopping = 10, tol = 1e-2, prob_tol_filter = None):
    '''
...
def geo_kernel_matrix(manifold_samples, n_neighbors = 20, C = 1e10):
    '''
