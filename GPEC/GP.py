...
class weighted_geodesic(gpytorch.kernels.Kernel):
    def __init__(self, manifold_samples, geo_matrix, lam = .1, rho = .1, kernel_normalization = False, **kwargs):
        ...
        self.exp_geo_kernel = utils_torch.exp_kernel_func(geo_matrix, lam = self.lam, q = 1)
    ...
    def forward(self, x1, x2, diag = False, **kwargs):
        ...
        # Calculate weighted distribution q(m | x, \rho)
        q1 = self.q_weighting(x1, self.manifold_samples, self.rho)
        if x1_eq_x2:
            q2 = q1
        else:
            q2 = self.q_weighting(x2, self.manifold_samples, self.rho)

        # Apply weighting
        output = q1 @ self.exp_geo_kernel @ q2.transpose(1,0)
...
def train_GPEC(x_train, y_train, manifold_samples = None, geo_matrix = None, kernel = 'WEG', var_list = None, n_iter = 10, lam = 0.1, rho = 0.1, kernel_normalization = True, batch_shape = torch.Size([]), lr = 0.1, optimizer = 'Adam'):
    ...
    if var_list is None:
        ...
        likelihood = gpytorch.likelihoods.FixedNoiseGaussianLikelihood(var_list, learn_additional_noise=False,)
