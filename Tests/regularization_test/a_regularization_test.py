...
try:
    ...
    warnings.warn('Using the wrong env')
...
parser.add_argument('--method', type = str,default='fmnist',
...
parser.add_argument('--learn_noise', type = int,default=1, help='learn GP noise')
parser.add_argument('--nn_softplus_beta', type = float,default=0.25, help='softplus beta parameter for nn training')
...
if args.explainer == 'kernelshap' and args.method not in ['minst', 'fmnist']:
    output_shape = 'singleclass'
else:
    output_shape = 'multiclass'

gpec_explainer_list = ['kernelshap', 'lime']
geo_matrix = None
...
if args.method[:6] == 'census':
    ...
    # Load Data
    tr_data_im, tr_data_lab, _, tr_loader, _, _, _, val_loader, te_data_im, te_data_lab, _, _ = load_mnist(nn_params)

    #####################
    # Train nn

    curdir = './Files/Models/Images/MNIST-Trained-Model%s' % str(nn_params.l2_reg)
    utils_io.make_dir(curdir)

    device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")
    print('device: %s' % device)
    model = SimpleANN(**nn_params.__dict__).to(device)
    model, train_accy, test_accy = train_network(model, nn_params.epochs, tr_loader, val_loader, nn_params.lr, nn_params.optimizer, nn_params.milestones, nn_params.lr_gamma, curdir, nn_params.l2_reg)
    ...
    # assume data is n x d
    if args.kernel == 'WEG' and args.explainer in gpec_explainer_list:
        # get fitlered samples
        seed_samples = decision_boundary.advsamples_pytorch(model, tr_data_im, tr_loader, nn_params)
        ...
        for target_class in classes:
            ...
            seed_samples_tmp = utils_torch.tensor2numpy(seed_samples_tmp)
            manifold_samples = decision_boundary.sampledb_DBPS_multiclass(seed_samples_tmp, target_class = target_class, model = model, n_samples_per_class = 100, batch_size = 4096)
            geo_matrix = decision_boundary.geo_kernel_matrix(manifold_samples)
    ...
    # Load Data
    tr_data_im, tr_data_lab, _, tr_loader, _, _, _, val_loader, te_data_im, te_data_lab, _, _ = load_fashionmnist(nn_params)
    ...
    if args.kernel == 'WEG' and args.explainer in gpec_explainer_list:
        # get fitlered samples
        seed_samples = decision_boundary.advsamples_pytorch(model, utils_torch.tensor2cuda(tr_data_im), tr_loader, nn_params)
        # Do I calculate this over classes?
...
if args.explainer == 'kernelshap' or args.explainer == 'lime':
    ##############################################################
    '''
    ...
    if args.method == 'mnist' or args.method == 'fmnist':
        # use deepshap if using mnist
        explainer = explainers.deepshap(model, x_train) 
        exp_list = explainer(x_train)
        # output is a list of length c. Each element of list is an n x d matrix
    else:
    ...
    for target_class in range(n_classes):
        ...
        pred_var_list = []
        pred_ci_list = []
        data, labels = utils_torch.numpy2cuda(x_train), utils_torch.numpy2cuda(exp) 
        ...
        for i, batch_y in tqdm(enumerate(batched_labels), position = 0, desc = 'Batch Progress'):
            ...
            # Variance List
            min_var = torch.zeros_like(tmp_y) + 1e-10 # for numberical stability
            tmp_var_list = min_var
            if args.learn_noise == 1 or args.kernel == 'RBF': tmp_var_list = None
            ...
            model.eval()
            likelihood.eval()
            pred_list.append(model(utils_torch.numpy2cuda(x_test).double()))
            time_pred = time.time() - timestamp_start
            timestamp_start = time.time()
            
            # mean
            pred_mean_list.append(pred_list[i].mean.cpu().detach().numpy())
            time_mean = time.time() - timestamp_start
            timestamp_start = time.time()
            
            # variance
            pred_var_list.append(pred_list[i].variance.cpu().detach().numpy())
            ...
            pred_ci_list.append(ci_upper - ci_lower)
    ...
    ### CXPLAIN
        ...
        if args.method == 'mnist' or args.method == 'fmnist':
            ...
            tmp_y = utils_torch.tensor2numpy(F.one_hot(utils_torch.numpy2cuda(y_train), num_classes = 10))
            ...
            attributions, confidence = explainer.explain(x_test.reshape(-1,28,28, 1), confidence_level=0.95)
